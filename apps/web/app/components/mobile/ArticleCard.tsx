...
/**
 * ニュースカード最下部の評価バー。score をリスト内の最大値に対する相対幅で 1 本描く。
 * 展開部の ScoreBreakdown が持っていた内訳は tooltip に残す。
 */
function ScoreBar({ item, scale, color }: { item: BaseItem; scale: number; color: string }) {
  const w = Math.max(0, Math.min(100, (item.score / scale) * 100));
  const hasParts =
    ...
    >
      <div
        ...
        style={{
          ...
          height: 5,
          borderRadius: 999,
          background: `color-mix(in oklch, ${color} 10%, var(--bg-sunken))`,
          overflow: "hidden",
        }}
      >
